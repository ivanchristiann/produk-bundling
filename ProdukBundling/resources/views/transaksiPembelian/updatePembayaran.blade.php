...
<label for="tanggal"><strong>Tanggal Transaksi </strong>: {{ date('j F Y', strtotime($transaksiPembelians->tanggal))}}</label>
<label for="namaSupplier"><strong>Nama Supplier </strong>: {{$transaksiPembelians->namaSupplier}}</label>
<input type="hidden" name="idTransaksi" class="form-control" id="idTransaksi" value="{{$transaksiPembelians->id}}">
<div>
...
<form method="POST" action="{{route('transaksiPembelian.update', $transaksiPembelians->id)}}">
    @csrf
    @method("PUT")
    <table id="daftarDetailTransaksi" class="table table-striped" style="width:100%">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><input type="number" name="totalBayar" class="form-control" id="totalBayar" value="{{$subTotal}}" readonly></td>
            </tr>
                <td colspan="6"><button type="submit" class="btn btn-danger w-100" style="margin-top: 20px;" >Bayar</button></td>
            </tr>
            @endif
        </tbody>
    </table>
